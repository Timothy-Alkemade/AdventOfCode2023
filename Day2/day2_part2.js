...
input.forEach(line => {
    ...
    sets.forEach(set => {
        ...
        if (blueFound)
        {
            blue = Number(blueFound[0].match(numRegex)[0]);
        ...
        if (redFound)
        {
            red = Number(redFound[0].match(numRegex)[0]);
        ...
        if (greenFound)
        {
            green = Number(greenFound[0].match(numRegex)[0]);
    ...
    let power = blueMin * redMin * greenMin;
    totalSum += power;
});
