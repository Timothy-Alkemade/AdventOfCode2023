...
input = `Game 1: 8 green, 4 red, 4 blue; 1 green, 6 red, 4 blue; 7 red, 4 green, 1 blue; 2 blue, 8 red, 8 green
...
mapAllowed = new Map();
mapAllowed.set('red', 12);
mapAllowed.set('green', 13);
mapAllowed.set('blue', 14);
...
input.forEach(line => {
    ...
    gameText = line[0];
    cubeText = line[1].trim();
    
    const regex = /Game \d+/g;
    const found = gameText.match(regex);
    const id = Number(found[0].split(' ')[1]);
    ...
    sets.forEach(set => {
        ...
        const blueFound = set.match(blueRegex);
        let blue = 0;
        if (blueFound)
        {
            blue = blueFound[0].match(numRegex)[0];

            if (blue > mapAllowed.get('blue')) {
                gameAllowed = false;
            }
        }

        const redRegex = /\d+ red/g;
        const redFound = set.match(redRegex);
        let red = 0;
        if (redFound)
        {
            red = redFound[0].match(numRegex)[0];

            if (red > mapAllowed.get('red')) {
                gameAllowed = false;
            }
        }

        const greenRegex = /\d+ green/g;
        const greenFound = set.match(greenRegex);
        let green = 0;
        if (greenFound)
        {
            green = greenFound[0].match(numRegex)[0];

            if (green > mapAllowed.get('green')) {
                gameAllowed = false;
            }
        }
