...
input = `...............307............130..................969...601...186.........................................312....628..........878..........
......479#../..*..............................#.....*......*............../309.....484........................*......-..........+.....89....
...
for (let index = 0; index < input.length; index++) {
    ...
    let nums = line.match(numRegex);
    let numsIterator = line.matchAll(numRegex);
    let matches = [...numsIterator];
    let startIndexes = []
    ...
    if (nums) {
        for (let j = 0; j < nums.length; j++) {
            ...
            let startIndexNum = startIndexes[j];


            let indexBefore = startIndexNum - 1;
            let indexAfter = startIndexNum + num.length;

            let ck = checktokens(index, indexBefore, indexAfter)
...
function getTokens(lineIndex, indexBefore, indexAfter) {
    let tokens = [];

    let lineBefore = undefined;
    if (validTextIndex(input, lineIndex - 1)) {
        lineBefore = input[lineIndex - 1];
    }
    let currentLine = input[lineIndex];
    let lineAfter = undefined;
    if (validTextIndex(input, lineIndex + 1)) {
        lineAfter = input[lineIndex + 1];
    }

    for (let ind1 = indexBefore; ind1 <= indexAfter; ind1++) {
        if (validLineIndex(lineBefore, ind1)) {
            tokens.push(lineBefore[ind1]);
        }
    }

    if (validLineIndex(currentLine, indexBefore)) {
        tokens.push(currentLine[indexBefore]);
    }
    if (validLineIndex(currentLine, indexAfter)) {
        tokens.push(currentLine[indexAfter]);
    }

    for (let ind3 = indexBefore; ind3 <= indexAfter; ind3++) {
        if (validLineIndex(lineAfter, ind3)) {
            tokens.push(lineAfter[ind3]);
        }
    }
...
function checktokens(line, indexBefore, indexAfter) {
    let tokens = getTokens(line, indexBefore, indexAfter);
    x = validateTokens(tokens);
    return x
}


function validTextIndex(text, index) {
...
function validLineIndex(line, index) {
    if (!line) {
        return false;
    }
