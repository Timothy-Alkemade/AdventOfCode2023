...
for (let index = 0; index < input.length; index++) {
    ...
    for (let j = 0; j < gears.length; j++) {
        ...
        let numbers = getValidNumbers(index, gearIndex);

        if (numbers.length == 2) {
            let toAdd = numbers[0].value * numbers[1].value;
            sum += toAdd;
        }
...
function getValidNumbers(lineIndex, gearIndex) {
    let numbers = getSurroundNumbers(lineIndex);
    let valids = [];
    ...
    numbers.forEach(number => {
        ...
        // check adjacent
        if (indices.includes(gearIndex) || indices.includes(gearIndex - 1) || indices.includes(gearIndex + 1)) {
            valids.push(number);
        }
    });
...
function getSurroundNumbers(lineIndex) {
    let numbers = [];
    
    if (lineIndex >= 0) {
        numbers = numbers.concat(getNumbers(lineIndex - 1));
    }
    numbers = numbers.concat(getNumbers(lineIndex));
...
function getNumbers(lineIndex) {
    let numbers = [];

    const numRegex = /\d+/g;

    num1 = [...input[lineIndex].matchAll(numRegex)];

    num1.forEach(num => {
        ratio = {}

        ratio.value = num[0];
        ratio.startIndex = num.index;
        ratio.endIndex = ratio.startIndex + num[0].length - 1;
