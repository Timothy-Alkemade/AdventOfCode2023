...
lines.forEach(line => {
    ...
    for (let index = 0; index < line.length; index++) {
        ...
        if (isNumber(char) && !startNumberFound)
        {
            startNumber = char;
        ...
        else if (isNumber(char) )
        {
            ...
            stringNumberToMatch += char;
            writtenNumber = isWrittenNumber(stringNumberToMatch);
            if (writtenNumber && !startNumberFound)
            {
                startNumber = map1.get(writtenNumber);
                startNumberFound = true;
                endNumber = map1.get(writtenNumber);
            }
    ...
    completeNumber = '' + startNumber + endNumber;
...
function isWrittenNumber(line) {
    string = '';
    for (let index = line.length - 1; index >= 0; index--) {
        const char = line[index];  
        string = char + string;

        if (matchesNumbers(string)) {
            return string;
        }
    }
    return undefined;
...
function matchesNumbers(stringNumberToMatch) {
    return map1.has(stringNumberToMatch)
}
